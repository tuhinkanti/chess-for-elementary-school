    ...
    try {
        ...
        for (const msg of messages) {
            ...
            if (typeof msg.role !== 'string' || !['user', 'assistant', 'system'].includes(msg.role)) {
                return res.status(400).json({ error: 'Invalid or missing role in message' });
            }
            if (typeof msg.content !== 'string' || msg.content.length > 1000) {
                return res.status(400).json({ error: 'Message content must be a string under 1000 characters' });
            }
        }

        if (systemPrompt !== undefined && (typeof systemPrompt !== 'string' || systemPrompt.length > 2000)) {
            return res.status(400).json({ error: 'systemPrompt must be a string under 2000 characters' });
        }
