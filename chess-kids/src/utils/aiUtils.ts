...
export function validateTutorRequest(body: unknown): { valid: boolean; error?: string } {
    ...
    for (const msg of b.messages) {
        ...
        if (m.content.length > 1000) {
            return { valid: false, error: 'Message content exceeds maximum length (1000 characters)' };
        }
    }
    ...
    if ('systemPrompt' in b) {
        ...
        if (b.systemPrompt.length > 2000) {
            return { valid: false, error: 'System prompt exceeds maximum length (2000 characters)' };
        }
    }
